...
class GameOfLifeByEvents:
    ...
    def __init__(self, height, width):
        ...
        # Текущая обрабатываемая клетка (i,j) (cобытие типа I)
    ...
    def init_random_cells_grid(self, p=0.3, seed=None):
        """ Инициализация матричной сетки значениями 0/1 случайных клеток """
        self.p=p
        self.seed=seed
...
class FieldScanEvent(Event):
    ...
    def execute(self, model):
        ...
        # Добавление события типа II для зацикливания
...
class EventsList:
    ...
    def get_next(self):
        """ Извлечение событий в порядке очереди """
...
class Logger:
    ...
    def __init__(self, results_dir="results", game: GameOfLifeByEvents | None=None, save_frames=False):
        ...
        self.file.write(f"ИНФОРМАЦИЯ:\n"
                        f"[LOG] Автор: 4199 Горшков Алексей Олегович (КНИТУ-КАИ им. А.Н.Туполева)\n"
                        f"[LOG] Запуск новой сессии\n"
                        f"[LOG] Дата и время: {timestamp}\n"
                        f"{'='*100}\n"
                        f"ПАРАМЕТРЫ:\n"
                        f"[LOG] Матричная сетка размерности: {game.height}x{game.width}\n"
                        f"[LOG] Инициализация живой клетки: p={game.p}\n"
                        f"[LOG] Инициализация мертвой клетки: q={1-game.p}\n"
...
class GameOfLifeByEventsApp:
    ...
    def __init__(self, game: GameOfLifeByEvents, fps=10, log=False, save_frames=False):
        ...
        # Цветовая палитра
        self.bg_color = (245, 248, 250)         # Холодный светлый цвет (задний фон)
        self.cell_color = (0, 120, 150)         # Сине-зеленый цвет (живые клетки)
        self.grid_color = (180, 200, 210)       # Мягкий серый цвет (линии сетки)
        self.text_color = (40, 40, 40)          # Почти черный цвет (текст)
        self.processing_color = (160, 160, 160) # Серый цвет (обрабатываемая клетка)
        ...
        pygame.display.set_caption("4199_ГоршковАлО_ConwaysGameOfLife")
